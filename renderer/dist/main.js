...
(() => {
  ...
  function from({ name, prefix, encode: encode3, decode: decode6 }) {
  ...
  // ../protocol/actions/src/libs/ipfs.factory.ts
  var catFile = async (cid) => {
    const ipfsApiUrl = `${IPFS_URL}/api/v0/cat?arg=${cid}`;
  ...
  var renderHTML = async (config2, templateConfig2, templateData2) => {
    try {
      ...
      const template = cleanTemplateString(templateString).replace(/^"/, "").replace(/"$/, "").replace(/(?<=>)"/g, "").replace(/"(?=<)/g, "");
      if (!template) {
